<template>
  <div class="comment" :class="{ reply: isReply }">
    <div class="comment__main">
      ...
      <div class="comment__main__body">
        <div class="comment___main__body__nickname__report">
          <div class="comment__main__body__nickname">
            {{ comment.nickname }}
          </div>
          <div
            v-if="isReportEnabled && !comment.byCurrentUser"
            class="comment___main__body__report"
          >
              ...
              <template v-if="comment.flagInfo">
                ...
                >
                  ...
                  <template v-if="study === 2 && condition === 1">
                    <p>
                      <strong
                        >The recommendation decision is made based on our
                        comment policy.</strong
                      >
                    </p>
                    <ul>
                      <li>
                        Commenters in this forum are instructed to be civil in
                        addressing and referencing other commenters and stay
                        focused on the subject at hand.
                      </li>
                      <li>
                        Comments represent a range of views and are judged the
                        most interesting or thoughtful.
                      </li>
                      <li>
                        In some cases, comments may be specifically highlighted
                        to showcase commentary from a particular region, or
                        readers with first-hand knowledge of an issue.
                      </li>
                    </ul>
                  </template>
                  <template v-else-if="study === 2 && condition === 2">
                    <p>
                      <strong
                        >The moderation decision is made based on our comment
                        policy.</strong
                      >
                    </p>
                    <ul>
                      <p>
                        Commentators in this forum are instructed to observe the
                        following rules:
                      </p>
                      <ul>
                        <li>
                          Abusive, defamatory, offensive or disparaging comments
                          on the basis of disability, ethnicity, gender, or
                          otherwise are forbidden.
                        </li>
                        <li>
                          Comments that are attacks or threaten another person,
                          promote violence, wish for harm to befall another
                          person are forbidden.
                        </li>
                        <li>
                          Stalking or harassing another person and any form of
                          discouraging participation by others are forbidden.
                        </li>
                        <li>
                          Misrepresentation of comments, user profiles, posting
                          advertisements, and all forms of spamming are
                          forbidden.
                        </li>
                        <li>
                          The forum administrators reserve the right to remove
                          any comment for any reason. They may also bar
                          individuals who break the above commenting rules.
                        </li>
                      </ul>
                    </ul>
                  </template>
                  <template v-else>
                    <p>
                      <strong
                        >Commentators in this forum are instructed to observe
                    ...
                    <ul>
                      ...
                      <li>
                        Comments that are attacks or threaten another person,
                        promote violence, wish for harm to befall another person
                        are forbidden.
                      </li>
                      <li>
                        Stalking or harassing another person and any form of
                      ...
                      <li>
                        Misrepresentation of comments, user profiles, posting
                        advertisements, and all forms of spamming are forbidden.
                      </li>
                      <li>
                        The forum administrators reserve the right to remove any
                        comment for any reason. They may also bar individuals
                        who break the above commenting rules.
                      </li>
                    </ul>
                  </template>
                </el-popover>
          ...
          <div v-if="comment.byCurrentUser" class="button" @click="showEditBox">
            Edit
          </div>
        </div>
        <div class="comment__main__body__content">
          {{ isReply ? comment.content : editedContent }}
        </div>
        <div class="comment__main__body__buttons" v-if="!comment.byCurrentUser">
          <div
            class="button"
            :class="{ active: isShowingReplyBox }"
          ...
          <div
            ...
            :disabled="!hasReplies"
            @click="toggleReplies"
            :class="{ active: isShowingReplies }"
          >
            Replies ({{ repliesCount }})
          </div>
          <div class="button" @click="voteLike">
            <img :class="{ active: like === true }" src="../assets/like.png" />
            ({{ likeCount }})
          </div>
          <div class="button" @click="voteDislike">
            <img
              :class="{ active: like === false }"
    ...
    <ReplyBox
      ...
      v-show="isShowingReplyBox"
      :fold="foldReplyBox"
    />
    <div
      v-if="!isReply && hasReplies && isShowingReplies"
    ...
    <el-dialog
      title="Edit"
      :visible="isShowingEditBox"
      center
      :before-close="hideEditBox"
    >
      <el-form>
...
export default {
  ...
  data() {
    ...
    if (this.savedData.reactions) {
      if (!isReply) {
        ...
        const found =
          this.savedData.reactions[this.parentComment.id] &&
          this.savedData.reactions[this.parentComment.id].replies[
            this.comment.id
          ];
        if (found) {
          like = found.like;
    ...
    flagInfoDisplay() {
      ...
      if (this.attitude === false) {
        return this.condition === 1
          ? "Disagree with the bot: this post is not insightful"
          : "Disagree with the bot: this post is not inappropriate";
      }

      return this.condition === 1
        ? "Agree with the bot: this is insightful"
        : "Agree with the bot: this is inappropriate";
    },
    reportInfoDisplay() {
      if (!this.reported) {
        return this.study === 1
      ...
      return this.study === 1
        ? "Reported"
        : this.condition === 1
        ? "Recommended: this is insightful"
  ...
  methods: {
    reactionsRef(endpoint) {
      return (
        (this.isReply
          ? `${this.user.id}/reactions/${this.parentComment.id}/replies/${this.comment.id}/`
          : `${this.user.id}/reactions/${this.comment.id}/`) + endpoint
      );
    },
    report() {
    ...
    unflag() {
      MessageBox.alert(
        `Thank you for your help. Your unflagging action has been reported${
          this.whom ? ` to the ${this.whom}` : ""
        }.`,
        undefined,
        {
          showConfirmButton: true,
    ...
    toggleReplies() {
      ...
      firebase
        .database()
        .ref(this.reactionsRef("viewReplies"))
        .push(new Date().toGMTString());
    },
    toggleReplyBox() {
    ...
    edit() {
      firebase
        .database()
        .ref(`${this.user.id}/replies/${this.comment.id}/content`)
        .push(this.editingContent);
